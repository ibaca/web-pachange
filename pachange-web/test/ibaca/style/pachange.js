...
function drawCalendarEvents() {
	...
	jQuery.getJSON(calendarUrl, function(data) {
		$.each(data.feed.entry, function(i, entry) {
			...
			// Generate content
			content = $("<div></div>");
			content.append("<h2>Evento d韆" + date.toDateString() + "</h2>");
			content.find("h2").append("<small>" + where + "</small>");
			content.append("<p/>").find("p").append("<div class='gmaps-container' data-where='" + where + "'></div>").append("Descipci髇: " + description + "<br>");
	...
	$("#local-menu").live("change", function(event) {
		...
		switch (toTab) {
			...
			case "map-partial":
				...
				subtitle = "Pr贸ximo evento"
				drawCalendarEvents();
				break;
			case  "subscription-partial":
				partial = "partials/subscription.html";
				subtitle = "Suscripci贸n a pr贸ximo evento"
				break;
			case "videos-partial":
				partial = "partials/videos.html";
				subtitle = "Galer铆a de v铆deos"
				break;
			case "info-partial":
			default:
				partial = "partials/info.html";
				subtitle = "Informaci贸n y contacto"
				break;
		}

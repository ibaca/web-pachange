...
(function($) {
						...
						if (options['showthumbnails']) {
							...
							for (var i=0; i<intResultsLength; i++) {
								if (options['displaytype'] === 'user') {
									_objThumbsHolder.append('<div><a href="'+entries[i].player['default']+'" target="_blank"><img src="' + entries[i].thumbnail.sqDefault + '" class="' + entries[i].id + '" /><br /><span class="thumbdescription">'+entries[i].title+'</span></a></div>');
	...
	function _loadYouTubeVid(strID, options, _objVideoNode) {
		return; //-- para pachange se ignora el player! (no nos gusta flash!)
